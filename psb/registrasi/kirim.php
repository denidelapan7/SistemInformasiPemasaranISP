<?php 
	...
	if(isset($_POST['submit'])){
		...
		if($insert){
			echo '<script>window.location="../registrasi/berhasil.php?id='.$generateId.'"</script>';
		}else{
...
try {                       
    ...
    $mail->setFrom('h.connectionsolusindo@gmail.com', 'Registrasi PSB');
     $mail->addAddress($email); 
        ...
        $mail->Body = $bandwith;
        $mail->send();
        header("location= ../registrasi/berhasil.php?id='.$generateId.'");
 
    }catch (Exception $e) {
    	header("location:../registrasi/index.php?alert=gagal"); 	
    }
